...
classdef PoseEstimator
    ...
    % This class handles human pose estimation using a pre-trained network
    ...
    properties (SetAccess=private)
        Network % An object providing an interface to the Network
    ...
    methods
        ...
        function keypoints = detectPose(obj,images)
            ...
            keypoints = obj.heatmaps2Keypoints(heatmaps);
        end
        ...
        function keypoints = heatmaps2Keypoints(obj,heatmaps)
            ...
            % TODO: implement sub-pixel estimation with neighbor pixels using bilinear
            % interpolation.
        ...
        function Iout = visualizeKeyPointsMultiple(obj,I,joints,bboxes)
            ...
            % Check dimensions
            if size(joints, 3) ~= size(bboxes, 1)
                error('Mismatch between joints and bounding boxes.');
            end
